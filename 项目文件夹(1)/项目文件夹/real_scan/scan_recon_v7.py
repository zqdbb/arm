#!/usr/bin/env python3
"""
V7 椅子重建: 深度几何分离（放弃逐帧颜色分割）
核心: 每帧提取盘面区域全部深度点 → 多帧累积 → Z高度+R圆柱过滤 → Poisson
"""
...
DEPTH_MIN_M = 0.38           # 相机Z下限（盘面~0.42m，椅子稍近，墙>0.5m）
DEPTH_MAX_M = 0.48           # 相机Z上限（墙在 0.5-0.67m，全部排除）
CHAIR_RADIUS_M = 0.035        # 椅子圆柱半径 3.5cm（椅子~2.3cm + 余量）
Z_PLATE_GAP = 0.005          # 盘面上方至少 5mm 才算椅子
Z_CHAIR_MAX = 0.10           # 椅子最高点距盘面 10cm
SEARCH_RADIUS_PX_MARGIN = 0  # 在 disc_radius_px 基础上扩大（0=精确使用盘半径）
...
def do_fusion(calib):
    """V7 融合: 深度几何分离"""
    print('\n' + '=' * 50)
    print('  V7 深度几何分离 + Poisson 重建')
    print('=' * 50)

    R_w2c = np.array(calib['R'])   # camera -> world 的旋转
    t_vec = np.array(calib['t'])   # camera -> world 的平移
    fx, fy = calib['fx'], calib['fy']
    ppx, ppy = calib['ppx'], calib['ppy']
    cW = calib['width']; cH = calib['height']
    step = calib.get('step_deg', STEP_DEG)
    disc_radius_m = calib.get('radius_m', 0.10)
    cx_disc = calib.get('cx', 0.0)
    cy_disc = calib.get('cy', 0.0)

    print(f'  盘面中心(世界): ({cx_disc*100:.1f}, {cy_disc*100:.1f})cm  半径={disc_radius_m*100:.1f}cm')
    ...
    color_files = sorted([f for f in (OUT / 'color').glob('*.png') if len(f.stem) == 3]
                         + [f for f in (OUT / 'color').glob('*.jpg') if len(f.stem) == 3])
    ...
    # 计算盘面中心在裁剪图中的投影
    # 注意: 标定 R,t 定义的 world→camera 是 p_c = R @ p_w + t
    p_w_disc = np.array([cx_disc, cy_disc, 0.0])
    p_c_disc = R_w2c @ p_w_disc + t_vec
    u_center = int(fx * p_c_disc[0] / p_c_disc[2] + ppx)
    v_center = int(fy * p_c_disc[1] / p_c_disc[2] + ppy)
    disc_radius_px = int(disc_radius_m / p_c_disc[2] * fx)
    search_radius_px = disc_radius_px + SEARCH_RADIUS_PX_MARGIN
    print(f'  盘面投影: ({u_center},{v_center})  盘半径={disc_radius_px}px  搜索半径={search_radius_px}px')

    # 预计算盘面 mask (所有帧共用)
    yy, xx = np.mgrid[0:cH, 0:cW]
    in_disc = (xx - u_center)**2 + (yy - v_center)**2 < search_radius_px**2

    print(f'\n逐帧提取深度点 ({n_frames} 帧)...')
    print(f'  深度范围: [{DEPTH_MIN_M:.2f}, {DEPTH_MAX_M:.2f}]m (相机Z) '
          f'圆柱R<{CHAIR_RADIUS_M*100:.0f}cm  Z板间隙>{Z_PLATE_GAP*1000:.0f}mm')
    t0 = time.time()
    ...
    for i in range(n_frames):
        ...
        if depth_u16 is None:
            frame_point_counts.append(0)
            continue

        depth_m = depth_u16.astype(np.float64) * 0.001

        # 盘面区域 + 深度范围 + 有效深度
        valid = (depth_m > 0) & in_disc & (depth_m > DEPTH_MIN_M) & (depth_m < DEPTH_MAX_M)
        n_valid = valid.sum()
        if n_valid < 50:
            frame_point_counts.append(0)
            if i < 3 or (i + 1) % 18 == 0:
                print(f'  [{i+1}/{n_frames}] {i*step:3d}deg  valid<50, skip')
            continue
        ...
        # 相机 -> 世界
        pts_world = (R_w2c.T @ (pts_cam - t_vec).T).T  # (N, 3)
        ...
        pts_obj = (R_z.T @ pts_centered.T).T  # (N, 3)

        all_pts_obj.append(pts_obj)
        frame_point_counts.append(len(pts_obj))
        ...
        if i < 3 or (i + 1) % 18 == 0:
            ...
            print(f'  [{i+1}/{n_frames}] {i*step:3d}deg  '
                  ...
                  f'R∈[{r_vals.min()*100:.1f},{r_vals.max()*100:.1f}]cm')

    valid_frames = sum(1 for c in frame_point_counts if c > 0)
    if valid_frames == 0:
        print('\n错误: 无有效帧')
        return

    merged = np.concatenate(all_pts_obj, axis=0)
    print(f'\n累积点合计: {len(merged):,}  (来自 {valid_frames}/{n_frames} 帧)')

    # ── 自适应 Z 方向判定 ──
    z_all = merged[:, 2]
    z_hist, z_edges = np.histogram(z_all, bins=200)
    z_peak_idx = np.argmax(z_hist)
    z_plate = (z_edges[z_peak_idx] + z_edges[z_peak_idx + 1]) / 2
    print(f'  盘面 Z 峰值: {z_plate*100:.2f}cm')

    # ── 椅子方向: 始终在盘面上方（正Z，靠近相机方向）──
    # 物理保证: 椅子坐在盘上，盘面 Z≈0，相机在 Z>0 方向
    chair_z_min = z_plate + Z_PLATE_GAP
    chair_z_max = z_plate + Z_CHAIR_MAX
    chair_side = '上方(正Z)'
    print(f'  椅子方向: {chair_side}  (盘面Z峰值={z_plate*100:.2f}cm)')
    print(f'  Z 过滤: [{chair_z_min*100:.1f}, {chair_z_max*100:.1f}]cm')

    # ── 用最远 Z 的点定位椅子 XY 中心 ──
    # 椅子在盘面上方延展最远，取 Z 最极端的前 20% 点求 XY 中心
    z_mask_for_center = (z_all > chair_z_min) & (z_all < chair_z_max)
    z_filtered = merged[z_mask_for_center]
    if len(z_filtered) > 500:
        z_extreme = np.abs(z_filtered[:, 2] - z_plate)
        top_20_idx = np.argsort(z_extreme)[-max(int(len(z_filtered) * 0.20), 200):]
        top_pts = z_filtered[top_20_idx]
        chair_cx = float(np.median(top_pts[:, 0]))
        chair_cy = float(np.median(top_pts[:, 1]))
        print(f'  椅子XY中心: ({chair_cx*100:.1f}, {chair_cy*100:.1f})cm '
              f'(距转轴 {np.hypot(chair_cx, chair_cy)*100:.1f}cm)')
    else:
        chair_cx, chair_cy = 0.0, 0.0
        print(f'  椅子XY中心: 默认(0,0) (Z过滤点数不足)')

    # ── Z + R 过滤 ──
    z_mask = (z_all > chair_z_min) & (z_all < chair_z_max)
    r_all = np.sqrt((merged[:, 0] - chair_cx)**2 + (merged[:, 1] - chair_cy)**2)
    r_mask = r_all < CHAIR_RADIUS_M

    chair_mask = z_mask & r_mask
    chair_pts_raw = merged[chair_mask]
    print(f'  Z+R过滤后: {len(chair_pts_raw):,} 点')

    if len(chair_pts_raw) < 200:
        print('错误: 过滤后点数太少')
        return
    ...
    pcd.points = o3d.utility.Vector3dVector(chair_pts_raw)
    ...
    if not valid_labels:
        print('DBSCAN 未找到有效簇，回退到全部过滤点')
        chair_pts = pts_arr
    else:
        # 椅子是离盘面最远的簇（盘面在 z_plate，椅子在盘面上方延展 ~9cm）
        best_label = max(valid_labels,
                         key=lambda l: abs(pts_arr[labels == l, 2].mean() - z_plate))
        chair_pts = pts_arr[labels == best_label]
        print(f'  选中离盘面最远簇 {best_label}: {len(chair_pts):,} 点 '
              f'Z_mean={chair_pts[:,2].mean()*100:.1f}cm')

    # ── 保存调试点云 ──
    pcd_dbg = o3d.geometry.PointCloud()
    pcd_dbg.points = o3d.utility.Vector3dVector(pts_arr)
    colors = np.zeros((len(pts_arr), 3))
    if valid_labels and len(pts_arr) > 0:
    ...
    log = {'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
           'method': 'V7_DepthGeometricSeparation',
           'n_frames': n_frames, 'valid_frames': valid_frames,
           'total_merged_pts': len(merged),
           'chair_pts_after_zr_filter': len(chair_pts_raw),
           'chair_pts_final': len(chair_pts),
           'voxel_size_mm': VOXEL_SIZE * 1000,
           'depth_range_m': [DEPTH_MIN_M, DEPTH_MAX_M],
           'chair_radius_m': CHAIR_RADIUS_M,
           'z_plate_gap_m': Z_PLATE_GAP,
           'z_chair_max_m': Z_CHAIR_MAX,
           'vertices': len(verts),
...
if __name__ == '__main__':
    ...
    # 检查 V6 数据目录（如果 V7 没有自己的数据）
    v6_depth = list((BASE / 'output/v6/depth').glob('*.png'))
    v7_depth = list((OUT / 'depth').glob('*.png'))
    if len(v7_depth) < 36 and len(v6_depth) >= 36:
        print(f'从 V6 复用深度数据 ({len(v6_depth)} 帧)')
        # 创建符号链接
        for sub in ['depth', 'color', 'depth_color']:
            v6_sub = BASE / 'output/v6' / sub
            v7_sub = OUT / sub
            if v6_sub.exists() and not any(v7_sub.iterdir()):
                for f in v6_sub.iterdir():
                    (v7_sub / f.name).symlink_to(f.resolve())
