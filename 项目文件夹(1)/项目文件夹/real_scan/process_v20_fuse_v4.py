...
print(f'Axis center (IR): {axis_origin*1000}')
print(f'Axis dir: {axis_dir}')

# ── 紧裁剪: 椅子 ~34x29px, 裁剪 80x60 留余量 ──
CROP_W, CROP_H = 80, 60
x1, x2 = W_s//2 - CROP_W//2, W_s//2 + CROP_W//2
y1, y2 = H_s//2 - CROP_H//2, H_s//2 + CROP_H//2
print(f'Crop: {CROP_W}x{CROP_H} @ [{x1}:{x2}, {y1}:{y2}]')
...
for i in range(0, n_frames, 8):
    ...
    roi = d[H_s//2-20:H_s//2+20, W_s//2-20:W_s//2+20]
    vd = roi[roi > 0]
    if len(vd) > 20:
        sample_depths.extend([np.percentile(vd, 10), np.percentile(vd, 90)])
d_min = np.percentile(sample_depths, 5) - 0.02
d_max = np.percentile(sample_depths, 95) + 0.05
print(f'Depth filter: {d_min*1000:.0f} - {d_max*1000:.0f} mm')
...
for i in range(n_frames):
    ...
    valid = (depth > d_min) & (depth < d_max)
    total_raw += (depth > 0).sum()
    total_filtered += valid.sum()
...
print(f'\nDepth filter: {total_raw}→{total_filtered} pts ({total_filtered/max(total_raw,1)*100:.1f}%)')
pts_all = np.vstack(all_pts)
print(f'Accumulated: {len(pts_all)} pts')
...
o3d.io.write_point_cloud(str(OUT / 'v20_v4_pcd.ply'), pcd_f)
...
o3d.io.write_triangle_mesh(str(OUT / 'v20_v4_mesh.ply'), mesh_f)
print(f'Saved: {OUT}/v20_v4_mesh.ply')
print('Done.')
