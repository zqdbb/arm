...
class GroupingOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        _, C, N = features.size()
        idx_flat = idx.reshape(B, -1).unsqueeze(1).expand(-1, C, -1).clamp(0, N - 1)
        output = features.gather(2, idx_flat).reshape(B, C, nfeatures, nsample)
...
class GatherOperation(Function):
    ...
    def forward(ctx, features: torch.Tensor, idx: torch.Tensor) -> torch.Tensor:
        ...
        output = features.gather(2, idx_expanded)
    ...
    def backward(ctx, grad_out):
        ...
        grad_out_data = grad_out.data.contiguous()
        idx_expanded = idx.unsqueeze(1).expand(-1, C, -1)
        grad_features.scatter_add_(2, idx_expanded, grad_out_data)
        return grad_features, None
...
def _ball_query_cpu(radius, nsample, xyz, new_xyz):
    ...
    for b in range(B):
        d = torch.cdist(new_xyz[b], xyz[b])
        d_sorted, d_idx = d.sort(dim=1)
        for i in range(npoint):
            valid = d_sorted[i] < radius
            n_valid = valid.sum().item()
            if n_valid > 0:
                idx[b, i, :min(n_valid, nsample)] = d_idx[i, valid][:nsample]
    return idx
